...
3. **No standalone "Case 7" page**, same reasoning as Conflict Has a Winner's AS-BUILT doc: the Lab Pack's locked numbering convention calls the rating/badge page "Handoff," not a Case, and that convention (from the coverage map, which is authoritative) supersedes the storyboard's internal "Case 7" label.
